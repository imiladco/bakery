...
final class Order_History extends Widget_Base
{
    ...
    protected function register_controls(): void
    {
        ...
        $this->register_confirm_modal_controls([
            ...
            'title' => __('لغو شد', 'bakery-widgets'),
    ...
    private function register_cancel_controls(): void
    {
        $this->start_controls_section('section_cancel', [
            'label' => __('لغو شد', 'bakery-widgets'),
            'tab' => Controls_Manager::TAB_CONTENT,
        ]);
        ...
        $this->add_control('cancel_text', [
            ...
            'type' => Controls_Manager::TEXT,
            'default' => __('لغو شد', 'bakery-widgets'),
        ]);
    ...
    private function register_stepper_style_controls(): void
    {
        ...
        $this->add_control('stepper_notice', [
            ...
            'raw' => __('مرحله از وضعیت خودِ سفارش در ووکامرس خوانده می‌شود. «در حال انجام» یعنی سفارش ثبت شد، «تکمیل‌شده» یعنی تحویل داده شد، و دو وضعیت «درحال آماده سازی» و «آماده تحویل» را همین افزونه به ووکامرس اضافه کرده. در سفارش لغوشده همان چهار موقعیت می‌مانند، ولی «لغو شد» جای مرحلهٔ پایانی می‌نشیند و مرحله‌های میانی طی‌نشده می‌مانند.', 'bakery-widgets'),
    ...
    /**
     * مراحل سفارش (فیگما، نودهای 289:7463، 289:7495 و 291:7525).
     *
     * فهرست مراحل از طرح دسکتاپ می‌آید و بس — چهار مرحله. طرح موبایل
     * (298:7555) مرحلهٔ پنجمی هم نشان می‌دهد که عمداً پیاده نشده: آن
     * نود مرجع استایل ریسپانسیو است، نه مرجع اینکه سفارش چند مرحله
     * دارد.
     *
