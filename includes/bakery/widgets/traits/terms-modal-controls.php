...
/**
 * محتوا/استایل/رندر مشترک مودال «قوانین و مقررات» — هم پشت ویجت مستقل
 * Terms_Modal (که خودش با بارگذاری صفحه باز می‌شود) و هم تعبیه‌شده
 * داخل ویجت Login (که بعد از تأیید کد OTP، پیش از انتقال به صفحهٔ
 * اصلی، همین مودال باز می‌شود) استفاده می‌شود — یک پیاده‌سازی، دو
 * زمینه، دقیقاً همان دلیلی که Account_Actions_Controls تریت شد.
 *
 * تفاوت دو زمینه فقط در پارامترهای render_terms_modal() است:
 *   - مستقل: $auto_show=true (از ابتدا نمایان)، $redirect_url=null
 *     (فقط قفل همین صفحه را باز می‌کند، جایی منتقل نمی‌کند).
 *   - تعبیه‌شده در Login: $auto_show=false (با ویژگی HTML `hidden`
 *     رندر می‌شود؛ assets/js/bakery-login.js با کلیک روی «تأیید و
 *     ورود» آن را نمایان می‌کند)، $redirect_url = آدرس تنظیم‌شدهٔ ورود
 *     (assets/js/bakery-terms-modal.js بعد از تأیید به آن‌جا می‌رود).
 */
trait Terms_Modal_Controls
{
    /* =====================================================================
     * ثبت کنترل‌ها — تب محتوا
     * =================================================================== */

    private function register_terms_modal_content_controls(): void
    {
        $this->register_terms_modal_header_controls();
        $this->register_terms_modal_terms_controls();
        $this->register_terms_modal_acceptance_controls();
        $this->register_terms_modal_behavior_controls();
    }
    ...
    private function register_terms_modal_style_controls(): void
    {
        $this->register_terms_modal_overlay_style_controls();
        $this->register_terms_modal_card_style_controls();
        $this->register_terms_modal_header_style_controls();
        $this->register_terms_modal_divider_style_controls();
        $this->register_terms_modal_term_row_style_controls();
        $this->register_terms_modal_acceptance_style_controls();
        $this->register_terms_modal_button_style_controls();
    }
    ...
    private function register_terms_modal_header_controls(): void
    {
        $this->start_controls_section('section_terms_header', [
            'label' => __('سربرگ قوانین', 'bakery-widgets'),
    ...
    private function register_terms_modal_terms_controls(): void
    {
    ...
    private function register_terms_modal_acceptance_controls(): void
    {
        $this->start_controls_section('section_terms_acceptance', [
        ...
        $this->add_control('terms_button_text', [
    ...
    private function register_terms_modal_behavior_controls(): void
    {
        $this->start_controls_section('section_terms_behavior', [
            'label' => __('رفتار قوانین', 'bakery-widgets'),
            'tab' => Controls_Manager::TAB_CONTENT,
        ]);

        $this->add_control('terms_behavior_notice', [
            'type' => Controls_Manager::RAW_HTML,
            'raw' => __('این مودال دکمهٔ بستنی ندارد — فقط با علامت‌زدن چک‌باکس و کلیک روی دکمه بسته می‌شود. بعد از پذیرفتن، در همان مرورگر (localStorage) به خاطر سپرده می‌شود تا دوباره پرسیده نشود؛ اگر متن قوانین را عوض کردید، «نسخهٔ قوانین» زیر را عوض کنید تا دوباره از همه پرسیده شود.', 'bakery-widgets'),
    ...
    private function register_terms_modal_overlay_style_controls(): void
    {
        $this->start_controls_section('section_style_terms_overlay', [
            'label' => __('پردهٔ قوانین', 'bakery-widgets'),
    ...
    private function register_terms_modal_card_style_controls(): void
    {
        $this->start_controls_section('section_style_terms_card', [
            'label' => __('کارت قوانین', 'bakery-widgets'),
            'tab' => Controls_Manager::TAB_STYLE,
        ]);

        $selector = '{{WRAPPER}} .bkw-terms-modal';

        $this->add_responsive_control('terms_card_max_width', [
        ...
        $this->add_responsive_control('terms_card_max_height', [
        ...
        $this->add_responsive_control('terms_card_padding', [
        ...
        $this->add_responsive_control('terms_card_gap', [
        ...
        $this->add_responsive_control('terms_card_radius', [
    ...
    private function register_terms_modal_header_style_controls(): void
    {
        $this->start_controls_section('section_style_terms_header', [
            'label' => __('سربرگ قوانین', 'bakery-widgets'),
            'tab' => Controls_Manager::TAB_STYLE,
        ]);

        $this->add_responsive_control('terms_header_icon_size', [
        ...
        $this->add_control('heading_terms_title', [
            'label' => __('عنوان', 'bakery-widgets'),
        ...
        $this->add_control('terms_title_color', [
            'label' => __('رنگ', 'bakery-widgets'),
            'type' => Controls_Manager::COLOR,
            'default' => '#231912',
        ...
        $this->add_control('heading_terms_subtitle', [
            'label' => __('زیرعنوان', 'bakery-widgets'),
            'type' => Controls_Manager::HEADING,
            'separator' => 'before',
        ]);
        ...
        $this->add_group_control(Group_Control_Typography::get_type(), [
            'name' => 'terms_subtitle_typography',
        ...
        $this->add_control('terms_subtitle_color', [
            'label' => __('رنگ', 'bakery-widgets'),
            'type' => Controls_Manager::COLOR,
            'default' => '#615249',
    ...
    private function register_terms_modal_divider_style_controls(): void
    {
        $this->start_controls_section('section_style_terms_divider', [
            'label' => __('خط جداکنندهٔ قوانین', 'bakery-widgets'),
            'tab' => Controls_Manager::TAB_STYLE,
        ]);

        $this->add_control('terms_divider_color', [
            'label' => __('رنگ', 'bakery-widgets'),
            'type' => Controls_Manager::COLOR,
            'default' => '#eaded6',
    ...
    private function register_terms_modal_term_row_style_controls(): void
    {
        $this->start_controls_section('section_style_terms_rows', [
            'label' => __('بندهای قوانین', 'bakery-widgets'),
            'tab' => Controls_Manager::TAB_STYLE,
        ]);
    ...
    private function register_terms_modal_acceptance_style_controls(): void
    {
        $this->start_controls_section('section_style_terms_acceptance', [
    ...
    private function register_terms_modal_button_style_controls(): void
    {
        $this->start_controls_section('section_style_terms_button', [
            'label' => __('دکمهٔ تأیید قوانین', 'bakery-widgets'),
            'tab' => Controls_Manager::TAB_STYLE,
        ]);
        ...
        $this->add_control('terms_button_text_color', [
            'label' => __('رنگ متن', 'bakery-widgets'),
            'type' => Controls_Manager::COLOR,
            'default' => '#ffffff',
        ...
        $this->add_control('terms_button_bg_color', [
        ...
        $this->add_control('terms_button_bg_disabled', [
        ...
        $this->add_responsive_control('terms_button_padding', [
            'label' => __('پدینگ', 'bakery-widgets'),
            'type' => Controls_Manager::DIMENSIONS,
            'size_units' => ['px'],
        ...
        $this->add_control('terms_button_radius', [
            'label' => __('رادیوس', 'bakery-widgets'),
            'type' => Controls_Manager::SLIDER,
            'size_units' => ['px'],
     ...
     * @param string|null $redirect_url وقتی پر باشد، بعد از تأیید به این آدرس
     *   می‌رود (مصرف Login)؛ خالی یعنی فقط قفل همین صفحه باز می‌شود (مصرف
     *   مستقل Terms_Modal).
     * @param bool $auto_show false یعنی با ویژگی HTML `hidden` رندر می‌شود و
     *   تا وقتی چیز دیگری (مثلاً assets/js/bakery-login.js) آن را نمایان
     *   نکند، دیده نمی‌شود.
     */
    private function render_terms_modal(array $settings, ?string $redirect_url = null, bool $auto_show = true): void
    {
        ...
        printf(
            '<div class="bkw-terms-modal-overlay" data-bkw-terms data-storage-key="%1$s" data-edit-mode="%2$s"%3$s%4$s>',
            esc_attr($storage_key),
            $is_edit_mode ? '1' : '0',
            null !== $redirect_url && '' !== $redirect_url ? ' data-redirect-url="' . esc_attr($redirect_url) . '"' : '',
            $auto_show ? '' : ' hidden',
        );
        ...
        // وقتی auto_show=false، لازم نیست: از ابتدا با hidden پنهان است.
        if (!$is_edit_mode && $auto_show) {
        ...
        printf(
            ...
            esc_html((string) $settings['terms_button_text']),
        );
        echo '</div>';
