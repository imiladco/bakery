 ...
 *
 * ⚠️ دامنه فقط فایل‌هایی‌ست که بازبینی شده‌اند. همین اشکال در بقیهٔ
 * ویجت‌ها هم هست، ولی آن سطح‌ها را کارفرما تأیید کرده و روشن‌کردنِ
 * نادیدهٔ همه‌شان می‌تواند ظاهرِ تأییدشده را عوض کند — هرجا پیش‌فرضِ
 * اعلام‌شده با استایل‌شیت اختلاف داشته باشد. روال این است: مقادیر هر
 * فایل یکی‌یکی با استایل‌شیت مقایسه می‌شوند، بعد روشن، بعد اسمش
 * این‌جا اضافه می‌شود.
 */
final class GroupControlDefaultsTest extends TestCase
{
    ...
    private const REVIEWED = ['header.php', 'cart-sidebar.php'];
